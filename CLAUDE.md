...
**Backend + all UI tracks (UI-1/2/3/4/5/6) + bot dispatch + full AI tier ladder + host-controlled bot fill + TRIBUTE-1 part D auto-tribute + Hard tier async wiring all shipped as of 2026-05-18.** Six autonomous sessions on 2026-05-18 took the project from "P0 logic shipped, needs infra" through "9 HTTP/SSE routes live + integration test green" to "React 19 game surface complete end-to-end with the move handler auto-dispatching three AI tier bots between human turns + auto-tribute + multi-round dealing":
...
- **Hard tier async** (commit `133ed0f`):
  ...
  - `MoveDeps` + `StartGameDeps` gain optional `generate` + `budget` fields, threaded through to `runBotsAsync`. Production wiring (Vercel AI Gateway client + `createUpstashBudget()`) lands with the deploy commit; tests inject deterministic fakes.

Remaining substantive work: AUTH-2 sibling-scorer KV migration (Critical Decision); TRIBUTE-1 part E manual command dispatch (`tribute_select` + `anti_tribute` still return invalid_move; UI substates S11/S12/S13 ready); production Vercel AI Gateway client wiring (`MoveDeps.generate` accepts client — needs route wrappers `api/room/[code]/{move,start}.ts` to inject the real LLM via `@ai-sdk` surface); `createUpstashBudget()` for production budget persistence.
...
- **Coverage gate**: 80% lines on `lib/**` enforced by `vitest run --coverage`. CORE-1 explicitly targets 95%+. Current suite: **922 tests** as of 2026-05-18 (after bot-fill + TRIBUTE-1 part D + Hard async session). Component tests use `// @vitest-environment jsdom` per-file directive + `tests/setup/jest-dom.ts` setup file.

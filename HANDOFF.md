...
- **TRIBUTE-1 realtime wiring** — server-side tribute logic exists (`lib/game/tribute.ts`) but `TributeRequiredEvent` / `AntiTributeEvent` / `TributeCompletedEvent` / `tribute_select` command aren't yet dispatched through the move handler. `src/screens/TributeModal.tsx` is ready to render when those events arrive.
- **Bot fill at game start** — host can't yet seat bots from the CreateRoom AI tier chips; the seat→tier mapping isn't persisted to RoomState. Once that lands, the in-handler `runBots` loop will fire automatically because it already reads `member.status==='bot'`.
- **Hard tier async wiring** — `runBots` falls back to medium when tier='hard' (synchronous path only). AI-2's `chooseHardMove` is a separate async dispatcher; wiring it through the move handler requires an async run-loop variant.

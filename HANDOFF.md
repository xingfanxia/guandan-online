# Handoff — guandan-online v1.0 (backend + UI-1/UI-2 + AI track shipped)

**Date**: 2026-05-18
**Status**: backend feature-complete + UI foundation + full AI tier ladder shipped. 9 HTTP/SSE routes plus the React 19 game surface (Card / Hand / Trick / Avatar primitives, OrientationLock + RotatePrompt, GameTable4P live-SSE screen) and three AI tiers (Easy + Medium with partner cooperation + Hard via Vercel AI Gateway). The session lifecycle is fully event-driven end-to-end: `room_joined` / `room_left` from lobby, `deal` from game-start, `move_played` / `move_passed` / `trick_won` / `round_end` / `game_end` from gameplay — all flowing through the single publishEvent gateway with per-recipient log keys and a contiguous version namespace across the lobby → game boundary so a single SSE `Last-Event-ID` resumes cleanly across phase transitions. Tests **768/768** · TS strict clean · grep-no-leak gate green · `main` synced to `origin/main`. The end-to-end integration test (`tests/integration/full-game-flow.test.ts`) walks create → join × 3 (with lifecycle events published) → start → SSE-subscribe → play → pass and asserts the full event sequence including lifecycle backlog. Remaining substantive work: UI-3 (landing), UI-4 (tribute), UI-5 (round-end / A-level / victory), UI-6 (6/8P layouts), AUTH-2 (Critical Decision — sibling KV migration), bot wire-up in `api/room/[code]/move` (the `computeBotMove` dispatcher exists but isn't yet invoked when next-turn-is-bot).
...
**Outstanding work**:
- **UI-3 / UI-4 / UI-5 / UI-6** — landing + room-create + waiting lobby (UI-3), tribute modal (UI-4), round-end / A-level / victory (UI-5), 6/8-player table layouts (UI-6). 4P table foundation is now playable end-to-end from the hash route; the remaining screens layer on top of the UI-1 primitives.
...
- **Bot wire-up in `api/room/[code]/move`** — `computeBotMove` dispatcher exists and is fully tested, but the move handler doesn't yet detect "next-turn-is-bot" and dispatch automatically. Easy/Medium are synchronous (cheap); Hard is async (3s budget). Roughly 1-2 hours of integration once the seat→tier mapping is decided.

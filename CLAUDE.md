...
**Backend + all UI tracks (UI-1/2/3/4/5/6) + bot dispatch + full AI tier ladder shipped as of 2026-05-18.** Five autonomous sessions on 2026-05-18 took the project from "P0 logic shipped, needs infra" through "9 HTTP/SSE routes live + integration test green" to "React 19 game surface complete end-to-end: landing → create → wait → 4P/6P/8P table → tribute → round-end / A-level / victory, with three AI tier bots that the move handler auto-dispatches between human turns":
...
- **In-handler bot run-loop**:
  - `lib/ai/runBots.ts` — synchronous loop wired into `lib/api/move.ts` after the human's move applies. While `round.currentTrick.currentPlayer` is a member with `status==='bot'`, computes + applies the bot's move (via `playCards`/`pass` directly), derives events through `deriveMoveEvent`, and appends to the events array with sequential versions. Also auto-calls `startTrick` between tricks (closes a latent gap where the move handler never started the next trick). Hard tier falls back to medium for now — async LLM dispatch lands when AI-2 wires through the handler. Round-end fanout (round_end/game_end) operates on the post-bot `advancedRound` so bot-triggered round finishes emit correctly.

Remaining substantive work: AUTH-2 sibling-scorer KV migration (Critical Decision); TRIBUTE-1 realtime wiring (server-side tribute logic exists in `lib/game/tribute.ts`, events not yet dispatched through the move handler); host-controlled bot fill at game start (CreateRoom AI chips don't yet persist seat→tier to RoomState); Hard tier async wiring through the move handler; `createUpstashBudget()` for production budget persistence.
...
- **Coverage gate**: 80% lines on `lib/**` enforced by `vitest run --coverage`. CORE-1 explicitly targets 95%+. Current suite: **880 tests** as of 2026-05-18 (after UI-3/4/5/6 + bot-dispatch session). Component tests use `// @vitest-environment jsdom` per-file directive + `tests/setup/jest-dom.ts` setup file.
